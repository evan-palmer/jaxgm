...
from jaxgm.linalg import frobnorm, logm_se3
...
def chordal_error(g: Num[Array, "n n"], h: Num[Array, "n n"]) -> DTypeLike:
    """Compute the chordal distance between two Lie group elements.
    ...
    -------
    DTypeLike
        The chordal distance between the two elements.
    ...
    """
    return frobnorm(g @ jnp.linalg.inv(h) - jnp.eye(g.shape[-1]))
...
def orthogonal_error(R: Float[Array, "n n"]) -> DTypeLike:
    ...
    """
    return frobnorm(R @ R.T - jnp.eye(R.shape[-1]))
