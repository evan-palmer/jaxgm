...
from jaxgm.linalg._norm import dampednorm, frobnorm
...
def sqrtm_pd(A: Num[Array, "n n"], tol: float = 1e-6) -> Num[Array, "n n"]:
    ...
    def cond(args):
        X, _ = args
        return frobnorm(jnp.linalg.matrix_power(X, 2) - A) ** 2 > tol
...
def _left_jac_inv_so3(w: Num[Array, "3"]) -> Num[Array, "3 3"]:
    ...
    jac = jnp.eye(3)
    return jax.lax.cond(
        jnp.isclose(dampednorm(w), 0.0),
        lambda: jac,
        lambda: jac
        - 0.5 * skew3(w)
        + (
            (1 / (dampednorm(w) ** 2))
            - (1 + jnp.cos(dampednorm(w)))
            / (2 * dampednorm(w) * jnp.sin(dampednorm(w)))
        )
        * (skew3(w) @ skew3(w)),
    )
